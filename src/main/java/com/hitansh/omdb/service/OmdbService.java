...
public class OmdbService {
    ...
    public void init() {
        if (apiKey == null || apiKey.trim().isEmpty()) {
            log.warn("OMDB API key is NOT set. Requests will fail.");
        } else {
            apiKey = apiKey.trim();
            String masked = "****" + apiKey.substring(apiKey.length() - 4);
            log.info("OMDB API key loaded (masked): {}", masked);
        }
    }
    ...
    private String buildSearchKey(String s, int page, String type) {
        return "search::" + normalize(s) + "::" + page + "::" + normalize(type);
    }

    private String buildMovieKey(String id) {
        return "movie::" + normalize(id);
    }
    ...
    public Mono<String> search(String s, int page, String type) {
        if (apiKey == null || apiKey.isBlank()) {
            return Mono.error(new IllegalStateException("OMDB API key missing"));
        }
        ...
        return Mono.fromCallable(() -> {
                    String cached = cache.getIfPresent(key);
                    if (cached != null) {
                        log.info("CACHE HIT ➜ search '{}', page {}", s, page);
                        return cached;
                    }

                    log.info("CACHE MISS ➜ Calling OMDB for search '{}', page {}", s, page);
    ...
    public Mono<String> getMovie(String id) {
        ...
        return Mono.fromCallable(() -> {
                    ...
                    String cached = cache.getIfPresent(key);
                    if (cached != null) {
                        log.info("CACHE HIT ➜ movie {}", id);
                        return cached;
                    }

                    log.info("CACHE MISS ➜ Calling OMDB for movie {}", id);
    ...
    public Optional<String> findImdbIdByTitle(String title) {
        try {
            String searchJson = search(title, 1, null)
